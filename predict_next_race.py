# predict_next_race.py  (fixed to produce q_delta_s + teammate_delta_q2)
import argparse
...
def predict_next(season:int, rnd:int, drivers_arg=None, qual_csv=None,
    ...
    # 3) rolling stats up to previous race
    rolling_stats = compute_rolling_stats(hist, up_to_season=int(season), up_to_round=int(rnd))

    # 4) assemble base rows with q_time_s and rolling stats
    rows=[]
    for c in candidates:
        drv = c['driver']; cons = c.get('constructor')
        row = {'driver': drv, 'constructor': cons}
        if drv in rolling_stats.index:
            row.update(rolling_stats.loc[drv].to_dict())
        else:
            row['avg_pos_last3'] = hist['race_position_num'].median()
            row['podiums_last3'] = 0.0
            row['points_last3'] = 0.0
        # set q_time_s from qtimes or historical median
        if drv in qtimes and not np.isnan(qtimes[drv]):
            row['q_time_s'] = qtimes[drv]
        else:
            row['q_time_s'] = hist['q_time_s'].median() if 'q_time_s' in hist.columns else 0.0
        rows.append(row)

    feat_df = pd.DataFrame(rows)

    # 5) compute q_delta_s (session normalization) using available q_time_s among candidates
    # if q_time_s missing for everyone (rare), fall back to hist median -> q_delta 0
    if feat_df['q_time_s'].notna().any():
        session_min = feat_df['q_time_s'].min(skipna=True)
        feat_df['q_delta_s'] = feat_df['q_time_s'] - session_min
    else:
        feat_df['q_delta_s'] = 0.0

    # 6) compute teammate_delta_q2: per constructor subtract team min q_delta
    ...
    # 7) final feature matrix for v3 model
    feature_cols = ['q_delta_s','teammate_delta_q2','avg_pos_last3','podiums_last3','points_last3']
    for col in feature_cols:
        if col not in feat_df.columns:
            feat_df[col] = 0.0
    X = feat_df[feature_cols].astype(float).fillna(0.0)

    dmat = xgb.DMatrix(X.values, feature_names=feature_cols)
    preds = model.predict(dmat)

    feat_df['prob_podium'] = preds
    feat_df = feat_df.sort_values('prob_podium', ascending=False).reset_index(drop=True)

    print(f"\nPredictions for Season {season} Round {rnd} (top 10):\n")
    print(feat_df[['driver','constructor','prob_podium']].head(10).to_string(index=False, justify='left'))
    return feat_df
...
if __name__ == "__main__":
    ...
    p.add_argument("--drivers", type=str, default=None)
    p.add_argument("--qual_csv", type=str, default=None)
    p.add_argument("--model", type=str, default="models/xgb_podium_v3.joblib")
    p.add_argument("--ml_table", type=str, default="data/ml_table_v3.parquet")
    args = p.parse_args()

    predict_next(args.season, args.round, drivers_arg=args.drivers, qual_csv=args.qual_csv,
                 model_path=args.model, ml_table_path=args.ml_table)
